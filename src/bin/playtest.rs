...
fn main() -> anyhow::Result<()> {
    ...
    println!("Commands: status, move <dir>, map, gather, forage, journey, pray, observe, offer, pilgrimage, rest [h], tend,");
    ...
    loop {
        ...
        match parts[0] {
            ...
            "observe" => {
                recorded.push(PlayerChoice::ObserveFestival);
                app.observe_festival();
                print_msg(&app);
            }
            "offer" => {
                recorded.push(PlayerChoice::MakeOffering);
                app.make_offering();
                print_msg(&app);
            }
            "pilgrimage" => {
                recorded.push(PlayerChoice::Pilgrimage);
                app.pilgrimage();
                print_msg(&app);
            }
            "gift" => {
            ...
            "folk" => {
                let idx: usize = parts.get(1).and_then(|s| s.parse().ok()).unwrap_or(0);
                if let Some(pos) = app.player_pos {
                    app.ask_folk(pos.region_idx, 0, idx);
                }
                print_msg(&app);
            }
            "life" => {
                let idx: usize = parts.get(1).and_then(|s| s.parse().ok()).unwrap_or(0);
                if let Some(pos) = app.player_pos {
                    app.ask_about_life(pos.region_idx, 0, idx);
                }
                print_msg(&app);
            }
            "council" => {
                let idx: usize = parts.get(1).and_then(|s| s.parse().ok()).unwrap_or(0);
                if let Some(pos) = app.player_pos {
                    app.ask_council(pos.region_idx, 0, idx);
                }
                print_msg(&app);
            }
            "province" => {
            ...
            "consult" => {
                let idx: usize = parts.get(1).and_then(|s| s.parse().ok()).unwrap_or(0);
                if let Some(pos) = app.player_pos {
                    app.consult_healer(pos.region_idx, 0, idx);
                }
                print_msg(&app);
            }
            "guide" => {
                let idx: usize = parts.get(1).and_then(|s| s.parse().ok()).unwrap_or(0);
                if let Some(pos) = app.player_pos {
                    app.hire_guide(pos.region_idx, 0, idx);
                }
                print_msg(&app);
            }
            "scribe" => {
                let idx: usize = parts.get(1).and_then(|s| s.parse().ok()).unwrap_or(0);
                if let Some(pos) = app.player_pos {
                    app.consult_scribe(pos.region_idx, 0, idx);
                }
                print_msg(&app);
            }
            "apprentice" => {
