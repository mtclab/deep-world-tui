...
impl App {
    ...
    pub fn start_build(&mut self) {
        ...
        let total_hours = kind.build_hours().max(1);
    ...
    pub fn rest(&mut self) {
        ...
        let structure_bonus = self.structure_at_player().map_or(0.0, |s| match s.kind {
        ...
        if self.god_affinity.get(GodName::Kukri) > 0.5 {
            ...
            if structure_bonus > 0.0 {
                self.status_msg = Some(format!(
                    "Rested deeply in a {}. Dreams of clear water. (8h)",
                    self.structure_at_player()
                        .map(|s| s.kind.label())
                        .unwrap_or("camp")
                ));
            } else {
